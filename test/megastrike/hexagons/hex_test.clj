...
(t/deftest multiplication
  (let [test-hex (sut/hexagon 4 0 -4)]
    (t/testing "Testing hex multiplication"
      (t/is (= (sut/multiplication test-hex 2) {:hex/p 8 :hex/q 0 :hex/r -8})))))
...
(t/deftest neighbor
  (let [test-hex-00 (sut/hexagon 0 0 0)
    ...
    (t/testing "Check each hex neighbor for hex 0,0"
      (t/is (sut/same-hex (nth neighbors 0) {:hex/p 1  :hex/q 0  :hex/r -1}))
      (t/is (sut/same-hex (nth neighbors 1) {:hex/p 1  :hex/q -1 :hex/r 0}))
      (t/is (sut/same-hex (nth neighbors 2) {:hex/p 0  :hex/q -1 :hex/r 1}))
      (t/is (sut/same-hex (nth neighbors 3) {:hex/p -1 :hex/q 0  :hex/r 1}))
      (t/is (sut/same-hex (nth neighbors 4) {:hex/p -1 :hex/q 1  :hex/r 0}))
      (t/is (sut/same-hex (nth neighbors 5) {:hex/p 0  :hex/q 1  :hex/r -1})))))
...
(t/deftest round
  (let [hex-00 (sut/hexagon 0 0 0)
        ppoint {:hex/p 0.2 :hex/q -0.3 :hex/r 0.1}
        qpoint {:hex/p -0.3 :hex/q 0.2 :hex/r 0.1}
        rpoint {:hex/p 0.1 :hex/q -0.3 :hex/r 0.2}
        rqpoint {:hex/p -0.3 :hex/q 0.1 :hex/r 0.2}
        elsepoint {:hex/p 0.2 :hex/q -0.2 :hex/r 0.0}]
...
(t/deftest facing
  (let [hex-42 (sut/hexagon 4 0 -4)
        ...
        sw-dest {:x 488 :y 350}
        nw-dest {:x 488 :y 250}]
    (t/is (= (sut/facing hex-42 n-dest layout) :direction/n))
    (t/is (= (sut/facing hex-42 ne-dest layout) :direction/ne))
    (t/is (= (sut/facing hex-42 se-dest layout) :direction/se))
    (t/is (= (sut/facing hex-42 s-dest layout) :direction/s))
    (t/is (= (sut/facing hex-42 sw-dest layout) :direction/sw))
    (t/is (= (sut/facing hex-42 nw-dest layout) :direction/nw))))
