...
If you want to run it from source, rather than compiling, follow steps 1-3 from the Manual Installation steps, but at step 4, you should run `clojure -M:run` or if you have just installed, `just run`.
...
In the Lobby Screen, set the Player for any (or all) of the forces to "Kevin". Kevin will then control those forces.
...
For the Combat Phase, you will need to click "Finish Attacks" for Kevin and "Next Turn" if Kevin is the last to attack so that you have time to review the results of the attacks before advancing to the End Phase.

### What Kevin can do

Kevin can move and shoot.

His movement algorithm is... Very rough. He'll probably generally move roughly in the direction of the enemy and/or cover. Most of the time. Also, he'll now never stand still.

His targeting algorithm is better, but not by much. He only makes regular attacks, no physical attacks or special attacks. He also will sometimes take impossible shots (i.e. ones who TN is Infinity because terrain is blocking the shot), but he should only do that if there are no possible shots. It hasn't occurred to him yet that he could just not fire every weapon every turn.
...
Like really bad. A big part of why I'm rushing this release is to get better information movement. If you see Kevin doing something particularly dumb, please take a screenshot of it and mark where he was and where you think he should have moved. I made some progress on Kevin's movement algorithm, but not a lot.
...
This release revises the AI and creates a structure to simplify adding things like movement modes.
...
This release will finally overhaul the UI and make it harder for users to "freeze" the game by misclicking. It will also address any issues with Kevin crashing the game. It will also split the game into a client-server model to allow for future network play. It will also create a proper save game and scenario format.

### R+2

This will be the actual client-server model with a first attempt at Network Play.
